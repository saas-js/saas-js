...
import { getConditionOperator } from '@saas-js/conditions'
...
export interface UseConditionChipOptions<
...
export interface ConditionChipApi<
  TDefinition extends AnyConditionsDefinition = AnyConditionsDefinition,
> {
 ...
 * the caller.
 */
export function useConditionChip<TDefinition extends AnyConditionsDefinition>(
  conditions: ConditionsController<TDefinition>,
  { condition, draft }: UseConditionChipOptions<TDefinition> = {},
): ConditionChipApi<TDefinition> {
  const [panel, setPanel] = useState<ConditionChipPanel | null>(null)
  const isDraft = Boolean(draft)
  const current = draft ?? condition

  const valueModeOf = (operator: string | undefined) =>
    operator
      ? getConditionOperator(conditions.definition, operator)?.valueMode
      : undefined

  const commitAndClose = () => {
    if (conditions.draft.commitDraft()) setPanel(null)
  }

  return {
    field: current?.field as DefinitionFieldId<TDefinition> | undefined,
    operator: current?.operator as DefinitionOperatorId<TDefinition> | undefined,
    value: current?.value,
    error: draft?.errors[0]?.message,
    isDraft,
    panel,
    openPanel(nextPanel) {
      if (!isDraft && condition) {
        conditions.draft.cancelDraft()
        conditions.draft.beginEditCondition(condition.id)
      }
      setPanel(nextPanel)
    },
    close() {
      setPanel(null)
      if (conditions.draft.get().draft) conditions.draft.cancelDraft()
    },
    selectField(field) {
      conditions.draft.updateDraft({ field })
      const nextOperator = conditions.draft.get().draft?.operator
      if (valueModeOf(nextOperator) === 'none') {
        commitAndClose()
      } else {
        setPanel('value')
      }
    ...
    selectOperator(operator) {
      conditions.draft.updateDraft({ operator })
      if (valueModeOf(operator) === 'none') {
        commitAndClose()
      } else {
        setPanel('value')
      }
    ...
    setValue(value) {
      conditions.draft.updateDraft({ value })
    },
    apply(patch) {
      const id = conditions.draft.commitDraft(patch)
      if (id) setPanel(null)
      return id
    },
    remove() {
      if (draft) {
        ...
        conditions.actions.remove(condition.id)
      }
    },
  }
}
